...
jQuery(function ($) {
	...
	const $inputClass = $('#wp-link-class');

	if (0 === $inputClass.length) {
		return;
	}

	const $inputBtn = $('#wp-link-btn');
	...
	function extendWpLink() {
		const _wpLink = $.extend({}, wpLink);

		wpLink.getAttrs = function () {
			const attrs = _wpLink.getAttrs();

			attrs.class = (
				' ' +
				$.trim($inputClass.val().toString()) +
				' '
			).replace(/ btn /g, ' ');
		...
		wpLink.buildHtml = function () {
			...
			let html = _wpLink.buildHtml(attrs);
		...
		wpLink.mceRefresh = function (searchStr, text) {
			...
			if (editor) {
				...
				if (linkNode) {
					classStr =
						' ' + editor.dom.getAttrib(linkNode, 'class', '') + ' ';
	...
	function extendAcfLink() {
		...
		$.each(acf.getFieldTypes(), function (i, v) {
