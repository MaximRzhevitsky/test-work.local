...
class WLD_WC_ACF_Location_Base {
	...
	public static function add_rule_type( array $groups ) : array {
		$group                             = __( 'WooCommerce', 'theme' );
		$groups[ $group ]['woo_page_type'] = __( 'Woo Page Type', 'theme' );
	...
	public static function add_rule_match( bool $result, array $rule, array $screen ) : bool {
		...
		if ( $post_id && 'woo_page_type' === $rule['param'] ) {
			if ( '==' === $rule['operator'] ) {
				$result = static::is_page( $post_id, $rule['value'] );
			} else {
				$result = ! static::is_page( $post_id, $rule['value'] );
			}
		}
	...
	public static function is_page( int $post_id, string $value ) : bool {
		return function_exists( 'wc_get_page_id' ) && wc_get_page_id( $value ) === $post_id;
	}
}
