...
class WLD_WC_Search_In_Menu_Base {
	public static ?string $old_after;
	...
	public static function add_form_in_item( string $title, $item, $args ) : string {
		if ( is_string( static::$old_after ) ) {
			$args->after       = static::$old_after;
			static::$old_after = null;
		}

		if ( in_array( 'search', $item->classes, true ) ) {
			static::$old_after = $args->after;
