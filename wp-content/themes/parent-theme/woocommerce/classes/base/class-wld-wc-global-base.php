 ...
 * @version 4.3.0
 */
class WLD_WC_Global_Base {
	public static function init() : void {
		add_filter( 'wld_hide_comments', '__return_false' );
		add_filter(
			'wld_header_classes',
			array( static::class, 'add_header_classname' )
		);
		add_filter(
			'page_template_hierarchy',
			array( static::class, 'set_default_template' )
		);
		add_filter(
			'wp_enqueue_scripts',
			array( static::class, 'disable_block_styles' )
		);
		add_filter(
		...
		add_filter(
			'do_shortcode_tag',
			array( static::class, 'wrapping_shortcodes' ),
			10,
			2
		);
		add_filter(
			'woocommerce_widget_cart_item_quantity',
			array( static::class, 'change_cart_item_quantity' ),
			10,
			3
		);
		add_action(
	...
	public static function add_header_classname( array $classes ) : array {
		if ( WLD_Is::woocommerce_enabled() ) {
			$classes[] = 'page-header_woocommerce';
		}
