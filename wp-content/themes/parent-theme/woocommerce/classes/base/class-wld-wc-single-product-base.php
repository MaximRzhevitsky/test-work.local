 ...
 * @version 4.3.0
 */
class WLD_WC_Single_Product_Base {
	public static bool $has_admin_fields = false;
	...
	public static function init() : void {
		...
		add_filter(
			...
			array( static::class, 'availability_text' ),
			10,
			2
		);
	}
	...
	public static function add_admin_tab( array $tabs ) : array {
		if ( static::$has_admin_fields ) {
			$tabs['theme'] = array(
				'label'    => esc_html__( 'Theme', 'theme' ),
	...
	public static function availability_text( string $text, WC_Product $product ) : string {
		if ( '' === $text && $product->is_in_stock() ) {
			$text = esc_html__( 'In stock', 'theme' );
		}
