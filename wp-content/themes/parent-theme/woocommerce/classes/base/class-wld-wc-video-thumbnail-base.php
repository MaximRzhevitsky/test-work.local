...
class WLD_WC_Video_Thumbnail_Base {
	public const KEY = '_thumbnail_id';

	public static function init() : void {
		add_filter(
			'wp_get_attachment_image_src',
			array( static::class, 'set_src' ),
			10,
			4
		);
		add_filter(
			'wp_get_attachment_image_attributes',
			array( static::class, 'set_attr' ),
			10,
			2
		);
		add_filter(
			'wp_prepare_attachment_for_js',
			array( static::class, 'set_js_data' ),
			10,
			2
		);
		add_filter(
	...
	public static function add_field( array $fields, WP_Post $post ) : array {
		if ( wp_attachment_is( 'video', $post ) && wp_doing_ajax() ) {
