...
class WLD_WC_Product_Badge_Base {
	...
	protected bool $is_custom_title;
	protected bool $allow_in_page;
	protected bool $allow_in_loop;
	...
	public function add() : void {
		$title = $this->is_custom_title ? esc_html__( 'Custom', 'theme' ) : $this->title;
		$badge = esc_html__( 'Badge', 'theme' );

		echo '<div class="options_group">';
		echo '<h2><strong>' . esc_html( $title . ' ' . $badge ) . '</strong></h2>';
		...
		if ( $this->allow_in_page ) {
			woocommerce_wp_checkbox(
				array(
					...
					'label' => esc_html__( 'Show in Page', 'theme' ),
				)
			);
		}
		if ( $this->allow_in_loop ) {
			woocommerce_wp_checkbox(
				array(
					...
					'label' => esc_html__( 'Show in Loop', 'theme' ),
				)
			);
		}
		echo '</div>';
	...
	public function show() : void {
		...
		if (
			...
			echo '<span class="badge badge-' . esc_attr( $this->key ) . '">' . esc_html( $title ) . '</span>';
		}
	}
}
