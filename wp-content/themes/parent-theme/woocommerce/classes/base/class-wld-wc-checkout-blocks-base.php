...
class WLD_WC_Checkout_Blocks_Base {
	protected static int $start_blocks   = 0;
	protected static int $current_block  = 0;
	protected static int $editable_block = 1;

	protected static bool $ajax_fragments = false;
	...
	public static function init() : void {
		...
		add_action(
			...
			array( static::class, 'ajax_get_block_format_content' )
		);
		add_action(
		...
		add_action(
			...
			array( static::class, 'order_review_wrapper_start' ),
			1
		);
		add_action(
			'woocommerce_checkout_after_order_review',
			array( static::class, 'order_review_wrapper_end' ),
			PHP_INT_MAX
		);
		add_filter(
			'woocommerce_checkout_registration_enabled',
			array( static::class, 'disable_registration_in_billing' )
		);
		add_filter(
		...
		add_action(
			...
			array( static::class, 'replace_products_in_review_order' ),
			PHP_INT_MAX
		);
		add_action(
			...
			static function () {
				...
				if ( ( $is_woo_ajax && ! static::$ajax_fragments ) || doing_action( 'woocommerce_checkout_order_review' ) ) {
					ob_start();
				}
			},
			1
	...
	public static function the_shipping_total() : void {
		$shipping_total = WC()->cart->get_shipping_total();

		if ( $shipping_total || isset( WC()->session->get( 'chosen_shipping_methods' )[0] ) ) {
			printf(
				'<tr class="cart-shipping-total"><th>%s</th><td>%s</td></tr>',
				esc_html__( 'Shipping:', 'theme' ),
				wp_kses(
	...
	public static function the_blocks() : void {
		...
		static::the_block_customer_details();
		static::the_block_shipping_details();
		static::the_block_shipping_options();
		while ( static::wrap_blocks( esc_html__( 'Payment', 'theme' ) ) ) {
			static::the_block_billing_details();
			static::the_block_payment_type();
		}
		static::the_block_create_account();
	...
	public static function the_block( array $args ) : void {
		...
		if ( static::$current_block === static::$editable_block && ( $args['content'] || $args['static'] ) ) {
			static::$editable_block ++;
			$class = 'block-done';
		} elseif ( static::$current_block > static::$editable_block ) {
			$class = 'block-empty';
		} else {
			$class = 'block-edited';
		}

		$edit_text = esc_html__( 'Change', 'theme' );
		$classes   = array(
			'block',
			'block-' . $args['type'],
			$class,
	...
	public static function wrap_blocks( string $title ) : bool {
		if ( 0 === static::$start_blocks ) {
			static::$start_blocks = static::$current_block;
			ob_start();
		...
		if ( static::$start_blocks > static::$editable_block ) {
			$class = 'block-empty';
		} elseif ( 0 === static::$editable_block ) {
			$class = 'block-done';
		} else {
			$class = 'block-edited';
		}
		...
		echo '<div class="blocks ' . esc_attr( $class ) . '"><h2 class="title">' . $title . '</h2>' . $blocks . '</div>';

		static::$start_blocks = false;
	...
	public static function the_block_shipping_details() : void {
		static::the_block(
			array(
				...
				'title'     => esc_html__( 'Delivery Address', 'theme' ),
				'action'    => 'block_shipping_details',
				'content'   => static::get_formatted_address( 'shipping' ),
				'save_text' => esc_html__( 'Ship to this Address', 'theme' ),
			)
		);
	}
	...
	public static function the_block_shipping_options() : void {
		static::the_block(
			array(
				...
				'title'  => esc_html__( 'Delivery Options', 'theme' ),
	...
	public static function the_block_billing_details() : void {
		static::the_block(
			array(
				...
				'title'     => esc_html__( 'Billing Address', 'theme' ),
				'action'    => 'block_billing_details',
				'content'   => static::get_formatted_address( 'billing' ),
				'sub_block' => true,
				'save_text' => esc_html__( 'Save Billing Address', 'theme' ),
			)
		);
	}
	...
	public static function the_block_payment_type() : void {
		static::the_block(
			array(
				...
				'title'     => esc_html__( 'Payment Type', 'theme' ),
				'action'    => 'block_payment_type',
				'content'   => '',
				'sub_block' => true,
				'static'    => true,
				'save_text' => esc_html__( 'Use this Card', 'theme' ),
			)
		);
	}
	...
	public static function the_block_create_account() : void {
		if ( ! is_user_logged_in() && WC()->checkout()->is_registration_enabled() ) {
			static::the_block(
				array(
					'type'   => 'create-account',
					'title'  => esc_html__( 'Create an Account', 'theme' ),
	...
	public static function ajax_get_block_format_content() : void {
		...
		if ( empty( $type ) || ! is_callable( array( static::class, 'the_block_' . $type ) ) ) {
			$errors->add(
				'invalid_block_type',
				esc_html__( 'Invalid Block Type', 'theme' )
			);
		}

		if ( 'customer_details' === $type ) {
			static::validate_customer_details( $fields, $errors );
		} elseif ( 'billing_details' === $type || 'shipping_details' === $type ) {
			$fieldset_key = 'billing_details' === $type ? 'billing' : 'shipping';
			static::validate_customer_address( $fieldset_key, $fields, $errors );
		}
		...
		if ( $errors->has_errors() ) {
			...
			call_user_func( array( static::class, 'the_block_' . $type ) );
	...
	public static function add_fragments( array $fragments ) : array {
		static::$ajax_fragments = true;

		ob_start();
		static::the_shipping_options();
	...
	public static function get_formatted_address( string $address_type ) : string {
		...
		if ( isset( $_POST['_ajax_get_block_format_content'] ) ) {
			...
			foreach ( $fieldset as $key => $field ) {
				$_key             = (string) str_replace( $address_type . '_', '', $key );
				$address[ $_key ] = WC()->checkout()->get_value( $key );
			}
		}
