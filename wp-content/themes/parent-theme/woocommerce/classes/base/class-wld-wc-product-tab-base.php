...
class WLD_WC_Product_Tab_Base {
	...
	protected mixed $show_title_in_tab;
	...
	public function show_fields() : void {
		...
		$tab   = esc_html__( 'Tab', 'theme' );

		echo '<div class="options_group custom-tab">';
		echo '<h2><strong>' . esc_html( $title . ' ' . $tab ) . '</strong></h2>';
		...
		if ( is_callable( $this->callback ) ) {
			...
			if ( $output ) {
				if ( $this->show_title_in_tab ) {
					echo '<h2>' . esc_html( $this->get_title( $product ) ) . '</h2>';
				}
	...
	protected function start_field( $field ) : void {
		...
		echo '<div class="form-field ' . esc_attr( $id ) . '_field ' . esc_attr( $class ) . '"><label for="' . esc_attr( $id ) . '">' . wp_kses_post( $label ) . '</label>';
		if ( $description && $tip ) {
			// todo: Escape output.
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
