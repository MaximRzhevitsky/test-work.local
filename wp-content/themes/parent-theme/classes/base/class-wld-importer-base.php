...
class WLD_Importer_Base {
	...
	public static function admin_init() : void {
		if ( static::$imports ) {
			...
			if ( wp_doing_ajax() ) {
				add_action( 'wp_ajax_wld_import_step', array( static::class, 'ajax_step' ) );
				add_action( 'wp_ajax_wld_import_stop', array( static::class, 'ajax_stop' ) );
			}
		}
	}
	...
	protected static function the_block( string $name, string $label, array $fields, int $count ) : void {
		...
		?>
		<div class="postbox wld-import-block hide-if-no-js <?php echo sanitize_html_class( $class ); ?>"
					...
					if (this && this.length) {
						this.each(function () {
							...
							function step() {
								...
								data.append('_ajax_nonce', '<?php echo esc_js( wp_create_nonce( 'wld_importer' ) ); ?>');
