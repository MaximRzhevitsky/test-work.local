...
class WLD_Log_Base {
	...
	public static function write( WP_Error|string $wp_error_or_message, string $type = 'error', string $handle = 'wld' ) : void {
	...
	public static function action_shutdown() : void {
		...
		if ( static::$has_changes ) {
			foreach ( static::$logs as $handle => $contents ) {
				WLD_Filesystem::put_file_contents( static::get_log_path( $handle ), $contents );
			}
		}
	}
	...
	protected static function get_log( $handle ) : string {
		if ( ! isset( static::$logs[ $handle ] ) ) {
			static::$logs[ $handle ] = WLD_Filesystem::get_file_contents( static::get_log_path( $handle ) );
		}
