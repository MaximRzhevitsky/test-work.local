...
class WLD_Images_Base {
	public static array $sizes            = array();
	public static array $bg_fit_endpoints = array( 320, 375, 768, 1024, 1440, 1600, 1920 );

	protected static string $svg_sprite = '';
	...
	public static function init() : void {
		...
		static::$sizes            = apply_filters( 'wld_image_init_get_sizes', $sizes );
		static::$bg_fit_endpoints = apply_filters(
			'wld_image_init_get_bg_fit_endpoints',
			static::$bg_fit_endpoints
		);

		foreach ( static::$bg_fit_endpoints as $endpoint ) {
			// Sizes for cover background endpoints
			static::$sizes[ $endpoint . 'x0' ] = array(
				'width'  => $endpoint,
				'height' => 0,
				'crop'   => false,
			);
	...
	public static function the_sprite() : void {
		if ( static::$svg_sprite ) {
			// todo: We need to figure out how to escape the SVG output.
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			echo '<svg width="0" height="0" style="display:none!important;">' . static::$svg_sprite . '</svg>';
		}
	}
		...
		if ( is_numeric( $attachment_id ) ) {
			...
			if ( $image ) {
				...
				if ( $image_meta ) {
					...
					if ( ! in_array( $current_size['width'], static::$bg_fit_endpoints, true ) ) {
						static::$bg_fit_endpoints[] = $current_size['width'];
					}

					if ( $image_meta['sizes'] ) {
						foreach ( $image_meta['sizes'] as $meta_size ) {
							if ( in_array( $meta_size['width'], static::$bg_fit_endpoints, true ) ) {
	...
	public static function get_bg_atts( ?int $attachment_id, string $size = 'full' ) : string {
		...
		if ( $x1 ) {
			$x2 = static::get_attachment_image_x2_url( $attachment_id, $size );
		}
