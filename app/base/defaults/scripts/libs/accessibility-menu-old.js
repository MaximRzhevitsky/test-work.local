export default function accessibilityMenu( menuNode ) {
	...
	function getIndexFirstChars( menuId, startIndex, char ) {
		for ( let i = startIndex; i < firstChars[ menuId ].length; i++ ) {
			if ( char === firstChars[ menuId ][ i ] ) {
				return i;
			}
		}
		return -1;
	...
	function setFocusByFirstCharacter( menuId, currentMenuitem, char ) {
		let start;
		let index;

		char = char.toLowerCase();
		start = menuitemGroups[ menuId ].indexOf( currentMenuitem ) + 1;
		if ( start >= menuitemGroups[ menuId ].length ) {
			start = 0;
		}

		index = getIndexFirstChars( menuId, start, char );
		if ( index === -1 ) {
			index = getIndexFirstChars( menuId, 0, char );
		}

		if ( index > -1 ) {
			setFocusToMenuitem( menuId, menuitemGroups[ menuId ][ index ] );
		}
	}
		...
		switch ( key ) {
			...
			default:
				if ( isPrintableCharacter( key ) ) {
					setFocusByFirstCharacter( menuId, tgt, key );
					flag = true;
				}
				break;
		}
	...
	function setTooltip( event ) {
		...
		if (
			...
			tgt.innerHTML +=
				'<span class="menu-tooltip" aria-hidden="true">use &#8592; &#8595; &#8594; to navigate</span>';
		}
	}
