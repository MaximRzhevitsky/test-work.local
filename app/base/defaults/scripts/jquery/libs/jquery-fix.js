...
/**
 * Added a hack to enable passive events.
 * elem.addEventListener( type, eventHandle, { passive: (type === 'scroll' ||type === 'mousewheel' || type === 'touchstart'? true : false)} );
 */
					...
					while ( soFar ) {
						...
						if ( ! matched || ( match = rcomma.exec( soFar ) ) ) {
							if ( match ) {
								...
								soFar =
									soFar.slice( match[ 0 ].length ) || soFar;
							}
							groups.push( ( tokens = [] ) );
						}
